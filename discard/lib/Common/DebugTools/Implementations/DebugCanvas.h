...
public:
	...
	ControlCanvas(const char* title) : MatForm(title) {
...
public:
	ControlCanvas(const char* title) { }
...
public:
	SimpleCanvas(const char* title) : MatForm(title) { }
...
public:
	ReadonlyCanvas(const char* title) : MatForm(title) { }
...
public:
	ReadonlyCanvas(const char* title) { }
...
#define MAKE_CONTROL_CANVAS(enable, name, arg) ControlCanvas<DEBUG_IMG && enable> name = arg;
#define MAKE_SIMPLE_CANVAS(enable, name, arg) SimpleCanvas<DEBUG_IMG && enable> name = arg;
#define MAKE_REFERENCE_CANVAS(enable, name, target) ReferenceCanvas<decltype(target), DEBUG_IMG && enable> name = target;
#define MAKE_READONLY_CANVAS(enable, name, arg) ReadonlyCanvas<DEBUG_IMG && enable> name = arg;
...
public:
	...
	MAKE_REFERENCE_CANVAS (1, armorNum, master)                // 装甲板识别结果
