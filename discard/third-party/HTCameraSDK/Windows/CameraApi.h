...
/// \brief 初始化一次录像。
/// \param [in] hCamera 相机的句柄。
/// \param [in] iFormat 录像的格式（0:不压缩  1:MSCV方式压缩  4:H264）
...
/// @ingroup API_TRIGGER
/// \~chinese
/// \brief 设置相机外触发信号的消抖时间。默认为0，单位为微秒。
/// \param [in] hCamera 相机的句柄。
/// \param [in] uTimeUs 时间
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// \param [in] pFrameBuffer 帧数据
/// \param [in] pFrameHead 帧头
/// \param [in] Left 矩形区域的起始x坐标
/// \param [in] Top 矩形区域的起始y坐标
/// \param [in] Width 矩形区域的宽度
/// \param [in] Height 矩形区域的高度
/// \param [out] AvgGray 返回计算结果
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
