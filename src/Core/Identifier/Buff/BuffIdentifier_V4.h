...
public:
	...
	std::optional<Buff5PointIdentifyData> Identify(const cv::Mat& img, const TimeStamp& timeStamp, const ArmorColor enemyColor) {
		_buffPlate5Points[4].x = 0; // return nullopt if onnx has no result
		_buffRPoint.x = 0;
...
//		cv::waitKey(1);
        ...
        if constexpr (debugCanvas.buff) {
            cv::circle(ROIimg, _buffPlate5Points[0], 2, COLOR_RED, 3, 8);
            cv::circle(ROIimg, _buffPlate5Points[1], 2, COLOR_RED, 3, 8);
            cv::circle(ROIimg, _buffPlate5Points[2], 2, COLOR_RED, 3, 8);
            cv::circle(ROIimg, _buffPlate5Points[4], 2, COLOR_RED, 3, 8);
            cv::circle(ROIimg, _buffRPoint, 2, COLOR_RED, 3, 8);
...
//		_pdata << std::endl << std::endl << std::endl;
		...
		if (_last_angle.timeStamp != -1) {
            ...
            if constexpr (debugCanvas.buff) {
                cv::circle(debugCanvas.buff.GetMat(), _buffPlate5Points[0], 4, COLOR_RED, 3, 8);
                cv::circle(debugCanvas.buff.GetMat(), _buffPlate5Points[1], 4, COLOR_RED, 3, 8);
                cv::circle(debugCanvas.buff.GetMat(), _buffPlate5Points[2], 4, COLOR_RED, 3, 8);
                cv::circle(debugCanvas.buff.GetMat(), _buffPlate5Points[4], 4, COLOR_RED, 3, 8);
            }
