...
public:
	...
	void Update(const Buff5PointIdentifyData& buff5PointIdentifyData, const unsigned short remainingTime,TransformerType transformer)
	{
        ...
        auto&& p = buffPnPSolver.SolvewithGyro(bp, transformer);
        position = *p;
	}

	//没有陀螺仪版本的更新和pnp解算
	void Update(const Buff5PointIdentifyData& buff5PointIdentifyData, const GimbalAttitude gyrosAttitude, const unsigned short remainingTime) {
		//获取数据信息
		_angle = buff5PointIdentifyData.buffAngle;
		_rightUP = buff5PointIdentifyData.rightUp;
        _leftUP = buff5PointIdentifyData.leftUp;
        ...
        auto&& p = buffPnPSolver.Solve(bp, gyrosAttitude);
        position = *p;
	...
	//有陀螺仪版的预测
	template<typename TransformerType>
	const std::optional<cv::Point3f> PredictwithGyro(const double deltaTimeSec,TransformerType transformer,const bool showPoints)
	{
        ...
        Buff5Point bp;
        bp.Set(_rightUP, _leftUP, _leftLOW, _rightLOW, _rCenter);
        buffPnPSolver.Solve(bp, transformer, showPoints?COLOR_RED:COLOR_GREEN);
        ...
        const bool showPoints)
    {
        ...
        Buff5Point bp;
        bp.Set(_rightUP, _leftUP, _leftLOW, _rightLOW, _rCenter);
        buffPnPSolver.Solve(bp, gyrosAttitude, showPoints?COLOR_RED:COLOR_GREEN);
        ...
        if (auto&& p = buffPnPSolver.Solve(bp, gyrosAttitude)) {
            return *p;
        }
        else {
            return std::nullopt;
    ...
    GimbalAttitude CalcGimbalAttitude(cv::Point3f p) const {
        const float yawOffset = 0.0f, pitchLevel = 15.0, pitchOffsetLow = 6.0f, pitchOffsetHigh = 15.0f; // history offset data of Mecanum
